# PAYable Tokenize Payment IPG SDK
...
**1. Add Payable Checkout URL into your checkout page.**
...
**2.** Build your payment payload with the required fields.

**2.1.** Required parameters:

- `notifyUrl` - Public URL that receives payment status callbacks
- `returnUrl` - URL to redirect users after payment
- `logoUrl` - Logo shown inside the SDK
- `merchantKey` - PAYable Merchant ID
- `currencyCode` - Currency Code (LKR/EUR/GBP/USD)
- `checkValue` - Security hash (see formats below)
...
**2.3.** Recurring payment parameters (required when `paymentType = 2`):
...
```php
<?php
function getCheckValue($merchantKey, $invoiceId, $amount, $currency, $merchantToken) {
    $mToken = strtoupper(hash("sha512", $merchantToken));
    $payload = implode("|", [$merchantKey, $invoiceId, $amount, $currency, $mToken]);
...
```php
<?php
function getCheckValueToken($merchantKey, $invoiceId, $amount, $currency, $customerRefNo, $merchantToken) {
    $mToken = strtoupper(hash("sha512", $merchantToken));
    $payload = implode("|", [$merchantKey, $invoiceId, $amount, $currency, $customerRefNo, $mToken]);
    return strtoupper(hash("sha512", $payload));
}
?>
```

**2.5.** Sample form with tokenization (JavaScript):
...
public function validateWebhook($webhookData, $merchantToken)
{
    ...
    $calculatedCheckValue = strtoupper(hash('sha512', $val));
...
This PAYable IPG JavaScript SDK is provided by PAYable (Pvt) Ltd.
